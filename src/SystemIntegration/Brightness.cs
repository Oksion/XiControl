...
public static class Brightness
{
    ...
    ///
    /// <b>Синхронно — и это принципиально.</b> Ползунок шлёт значения потоком, и Task.Run на
    /// каждое ломал их порядок: задачи уходили в пул и исполнялись вразнобой, а каждая ещё и
    /// сверялась с Get. Спуск 60 → 55 → 50 → 45, где «45» успевало раньше «50», кончался тем,
    /// что задача с «50» видела несовпадение и поднимала яркость обратно. Побеждал не последний
    /// по замыслу, а последний по случайности. Звать только с фонового потока — писать пачку
    /// подряд обязан один поток, иначе порядок теряется снова.
    /// </summary>
    public static void SetAsUser(int percent)
    {
        try { Set(Math.Clamp(percent, 0, 100)); }
        catch (Exception ex) { Log.Ex("Brightness.SetAsUser", ex); }
    }
