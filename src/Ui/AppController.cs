...
public sealed class AppController
{
    ...
    /// <summary>
    /// Принять порог заряда из прошивки: единственный источник истины — она (docs/12), а внешнюю смену
    /// (Xiaomi PC Manager, чужая утилита) мы иначе не заметим — канала уведомления в MIFS нет. Зовётся
    /// в момент показа панели и пересборки меню трея, поэтому фонового опроса не появляется.
    /// <para>Прошивка молчит (<c>null</c>) → конфиг не трогаем: лучше показать своё значение, чем
    /// затереть его нулём. Процент принимаем только валидным пресетом (вслепую чужой уровень не пишем).</para>
    ...
    public bool SyncCareFromFirmware()
    {
        ...
        bool care = live < 100;
        bool changed = false;
        if (care != _cfg.ChargeCare) { _cfg.ChargeCare = care; changed = true; }
        // 100% означает «защита выключена», а не «порог 100»: выбранный X сохраняем до включения
        if (care && Mifs.ChargeCodeForPercent(live) is not null && live != _cfg.CareLimitPercent)
        {
            _cfg.CareLimitPercent = live;
            changed = true;
        }
        if (!changed) return false;

        Log.Write($"Заряд: принят порог прошивки — {(care ? $"беречь {live}%" : "100% (защита выключена)")}");
        _cfg.Save();
        return true;
    }
