...
public sealed class AppConfig
{
    ...
    /// <summary>Короткий щелчок мотора тачпада на шаге краевого ползунка (XIC-73) — как в
    /// Xiaomi PC Manager. Работает только там, где есть вендорский канал тачпада; включено по
    /// умолчанию, потому что без отклика непонятно, поймался ли жест.</summary>
    public bool TouchpadEdgeHaptics { get; set; } = true;

    /// <summary>Не чаще одного щелчка за столько мс (только правкой config.json, 50–500).
    /// 50 — каждая порция шагов: чаще не бывает, ползунок применяет шаги раз в 50 мс.
    ...
    /// лишь там, где DPTF нет вовсе. На TM2424 зона отдаёт заглушку (27,9 °C при 73 °C на
    /// доменах), поэтому заведомо неправдоподобное число здесь — признак, что путь исполнился.
    /// </summary>
    public bool ForceAcpiTemperature { get; set; } = false;
    ...
    /// <summary>
    /// Какие частоты участвуют в переборе по клавише (действие «hz»). Пусто или null — все,
    /// что отдаёт панель: так было до появления настройки, и так остаётся по умолчанию.
    ///
    /// Смысл в том, что панель отдаёт все свои режимы подряд (48/60/75/100/120), а нужны
    /// человеку обычно два-три: каждый лишний шаг — реальная смена видеорежима с чёрным кадром.
    /// </summary>
    public List<int>? CycleRefreshRates { get; set; }
    ...
    /// <summary>Какую метрику показывать в самом компактном виде («только число»): "power"
    /// (по умолчанию), "cpu", "gpu", "ram", "temp" — тот же список, что у индикатора в трее.
    /// Настройка своя: трей и виджет часто держат открытыми вместе, и показывать в них одно
    /// и то же бессмысленно. Выбирается правой кнопкой по виджету (XIC-70).</summary>
    public string? MonitorCompactMetric { get; set; }
    ...
    /// TM2424. Поддерживаются именованные слоты полного OEM-диспетчера: Mi down/up,
    /// projection, screenshot, taskView, settings, AI down/up, mic, backlight,
    /// touchpad state/toggle, Fn/Caps/Num Lock, performance, calculator, lowPower,
    /// refreshRate, winKeyLock и cameraPrivacy; значение — код в виде <c>"0x18"</c> (как в
    ...
    /// фолбэк через SetupAPI, переживает перезагрузку). Это БУХГАЛТЕРИЯ («гасили мы»), а не
    /// намерение: включать ли обратно на старте, решает <see cref="TouchpadKeepOff"/>.
    /// Ставится/снимается автоматически.</summary>
    public bool TouchpadPersistOff { get; set; }

    /// <summary>Оставлять тачпад выключенным между перезагрузками (XIC-53). По умолчанию
    /// false — прежнее поведение «выключение до перезагрузки».</summary>
    public bool TouchpadKeepOff { get; set; }
    ...
    /// фолбэк через SetupAPI, переживает перезагрузку). Это БУХГАЛТЕРИЯ («гасили мы»), а не
    /// намерение: включать ли обратно на старте, решает <see cref="TouchscreenKeepOff"/>.
    /// Ставится/снимается автоматически.</summary>
    public bool TouchscreenPersistOff { get; set; }

    /// <summary>
    /// Оставлять сенсорный экран выключенным между перезагрузками (XIC-53, issue #39).
    /// Человек, которому тачскрин не нужен вовсе, гасил его Диспетчером устройств — а мы
    /// включали обратно на каждом старте, потому что «выключено» у нас всегда означало
    /// «до перезагрузки». По умолчанию false: прежнее поведение никому не ломаем.
    /// </summary>
    public bool TouchscreenKeepOff { get; set; }
