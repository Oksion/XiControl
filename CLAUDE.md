...
вкл/выкл, мёртвая зона у нижнего края тачпада, сила вибрации и усилие нажатия тачпада,
сильное нажатие на тачпад как назначаемое действие,
...
- **`dotnet format` не гонять по файлам с ручным выравниванием** — формат его схлопывает:
  `src/Ui/OsdForm.cs` (колоночный маппинг иконок), `src/Wmi/Mifs.cs` (таблицы «код ↔ процент»),
  `src/Ui/TrayApp.cs` (висячий отступ лямбд), `src/Ui/Settings/SettingsToolkit.cs` (плотные
  инициализаторы), `src/SystemIntegration/AlsSensor.cs` (колонка комментариев COM-интерфейса),
  `src/SystemIntegration/PowerProfileGuard.cs` (`else` под `if`). Тот же список стоит в
  `--exclude` шага Format check в `ci.yml` — держать их в синхроне (XIC-52). Исходники —
  UTF-8 **без BOM** (`.editorconfig`).
...
- `src/Ui/Settings/` — начинка окна настроек: `SettingsToolkit` (фабрика виджетов: карточки,
  ...
  мёртвая зона снизу, края-ползунки, тактильный отклик — в отличие от «Функций», где только видимость) / `PerfTab` / `KeysTab` /
...
- `src/SystemIntegration/` — `ChargeGuard` (переустанавливает лимит заряда после сна/смены
  ...
  **классический COM Sensor API в elevated мёртв**, а стрим требует живого подписчика
  ...
  проходит — XIC-24),
  `TouchpadHaptics` + `TouchpadHapticsProtocol` (сила вибрации и порог нажатия тачпада BLTP7853,
  XIC-77: вендорская коллекция `COL05`, `HidD_SetOutputReport`/`GetInputReport` — канал Xiaomi
  PC Manager, протокол восстановлен из его `SvrCModule.dll`; кадры и сумма — чистый `Protocol`
  под тестами на живых байтах. Значения живут в самом тачпаде (переживают сон и ребут) — ни
  гарда, ни поля в конфиге: одно чтение на старте, запись только по выбору пользователя с
  проверкой чтением. Коллекция опознаётся ещё и по дескриптору, чтобы чужой пэд не получил ни
  байта. `Pulse()` — щелчок мотора (команда `0x02`, снята с плагина краевых жестов
  `PluginGesture.dll`, XIC-73): зовётся с воркера `TouchpadEdgeSliders` на шаге ползунка, не
  ждёт замка — если идёт запись настроек, щелчок пропускается; сила щелчка — команда `0x58`,
  тоже в самом тачпаде, частота — `TouchpadEdgeHapticsMs` в конфиге),
  `TouchpadInput` (XIC-78: **один** `RawTouchpadReader` на процесс — Raw Input даёт процессу одно
  окно-получатель на usage, второй читатель молча отобрал бы ввод у первого; счётчик
  пользователей Acquire/Release, давление разбирается только пока кому-то нужно — краевым
  ползункам хватает координат), `TouchpadHeavyPress` + `HeavyPressDetector` (сильное нажатие:
  Tip Pressure контакта с Confidence > 500, раз за касание — как в `PluginGesture.dll` PC Manager;
  на TM2424 клик ≈ 130, сильное ≈ 1000. Действие — через `KeyRouter.Run`, список общий с клавишами;
  второй щелчок прошивки `0x59` пишется только при явном вкл/выкл, с «Нет действия» не подписан
  и давление не читается),
  ...
  **Исходящее событие (XIC-75)** — `ChargeLimitWatcher` (опрос заряда; чистые и покрытые
  тестами `Target` — за каким порогом следим, аппаратным или программным (XIC-74), `Decide` —
  «один раз за зарядку» со взводом на отключении зарядника, и `ShouldRemind` — напоминания
  программного порога) + `Webhook`
...
08 — спецификация иконок, 09 — мониторинг питания, 15 — HTTP API (маршруты, коды, вебхук).
