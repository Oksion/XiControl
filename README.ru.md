...
- **Обучение можно выключить** (тумблер «Обучение кривой») — когда кривая уже настроена как
  нравится: правки яркости становятся *временными*. Кривая не меняется, а утилита через минуту
  начинает мягко возвращать яркость к выученному — тем же вежливым торгом, что у лимита
  (половина разрыва раз в минуту, в обе стороны). Настоял повторной правкой — уступит на
  2 часа или до блокировки экрана; после разблокировки экран снова у выученного уровня.
