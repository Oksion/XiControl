...
автозапуск, тема панелей, запоминание яркости, проверка обновлений, логирование), Функции
...
«слабого блока», здоровье батареи), Экран (авто-герцовка, «Удерживать частоту» и частоты;
вкладка скрыта, если управление частотой выключено), Тачпад (мёртвая зона у нижнего края),
Производительность (видимость режимов, режим при старте, профили питания), Клавиши
(переназначение), HTTP API (порт, токен, разрешения) и О программе. Быстрые переключатели
...
`travel`, `touchpad`, `touchscreen`, `projection`, `settings`, `copilot`, `launch`, `none`:
