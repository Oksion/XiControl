...
Выпадашки в настройках предлагают пресеты через 10%, но лимит принимает **любой процент**: впиши
в `config.json` хоть `"BrightnessCapBattery": 47` (диапазон 10–100) — окно настроек покажет твоё
значение первым пунктом списка и не перетрёт его, пока сам не выберешь другое.
