...
Сворачивается в компактную строку (Power / CPU / GPU / RAM) или в один крупный показатель — кнопкой
«вид» или двойным кликом; направление тока — цветом (заряд зелёный / разряд оранжевый).
В самом компактном виде показатель выбирается правой кнопкой: потребление, CPU, GPU, RAM или
температура — от сети, где ватт нет, можно смотреть загрузку или градусы.*
...
- 🔋 **Защита заряда** — «беречь батарею» с выбираемым порогом (40/50/60/70/80%, выбор в
  ...
  - ⏰ **Программный порог** — для моделей, где прошивка лимит **не принимает**: приложение
    следит за зарядом и говорит, что пора выдернуть провод (всплывашка + звук, до трёх
    напоминаний). Появляется в Настройки → Батарея только на таких машинах — и только после
    того, как прошивка отказала: списков моделей в коде нет, железо отвечает само. Это
    подсказка, а не защита; настоящую даёт **вебхук** с умной розеткой (см. HTTP API ниже).
...
- 📳 **Сила вибрации и усилие нажатия тачпада** — как в Xiaomi PC Manager, тем же каналом и
  без драйверов; настройки хранятся в самом тачпаде.
...
- ⚙️ **Окно «Настройки»** в стиле Windows 11 — все опции по вкладкам (Общие / Функции /
  Батарея / Экран / Тачпад / Производительность / Клавиши / Уведомления / HTTP API /
  О программе), тёмная и светлая темы.
...
Тачпад (мёртвая зона у нижнего края, края-ползунки, сила вибрации и усилие нажатия, сильное нажатие),
Производительность (видимость режимов — отдельно для сети и для батареи, переключатель наверху
списка; режим при старте, профили питания), Клавиши
(переназначение), Уведомления (положение подсказок на экране, сколько висят, какие из
клавиш-фиксаторов показывать), HTTP API (порт, токен, разрешения) и О программе (версия, модель с кодом
...
**Настройки → Производительность**: по одному тумблеру на режим, а над списком — переключатель
**«От сети / От батареи»**. Видимость своя у каждого источника: «Полную мощность» на батарее
прошивка всё равно не примет, а «Эко» в розетке нужен редко. Применяется сразу; панель при этом
не сжимается — ячейки оставшихся режимов растягиваются. Совсем пустым список не станет: минимум
один видимый режим считается отдельно для каждого источника.
...
**Режим, которого нет на твоей модели, прячется сам.** Если прошивка отказала в нём и от сети,
и от батареи, он убирается из списка — одного источника намеренно мало: «Полная мощность»
законно отказывает на батарее и работает от сети. Ничего не проверяется за твоей спиной:
приложение учится только на режимах, которые ты выбираешь сам.

В `%APPDATA%\XiControl\config.json` это `HiddenModesBySource` — по списку на каждый источник:

```json
"HiddenModesBySource": { "ac": ["Eco"], "battery": ["FullSpeed"] }
```

Старые поля `EcoMode`/`FullSpeedMode` и общий список `HiddenModes` мигрируют автоматически при
первом запуске новой версии — править их вручную больше не нужно. После правки конфига руками
...
- **Кривую можно править прямо на графике**, не открывая config.json: точку перетащить мышью,
  двойным кликом по пустому месту добавить новую, правой кнопкой по точке — убрать (от 2 до 12
  точек). То же с клавиатуры: ←/→ выбрать точку, ↑/↓ яркость (с Shift — крупнее), Ctrl+←/→
  освещённость, Insert и Delete. Правится кривая одного источника — того, что выбран
  переключателем «От сети / От батареи» над графиком; вторая видна бледной линией для
  сравнения. Точка не уходит за соседей: «светлее в комнате — темнее экран» кривая не умеет
  ни выучить, ни получить мышью.

Дружит с остальными фичами яркости: **лимит** (см. выше) работает фильтром поверх — кривая
учит твоё настоящее намерение, а лимит просто не пускает выше на выходе (на графике он
показан пунктирной чертой: всё, что выше неё, экран не покажет);
**«Запоминать яркость»** при включённой авто-яркости выключается —
...
- **Края тачпада как ползунки.** Провести пальцем вверх-вниз вдоль левого края — яркость,
  вдоль правого — громкость (Настройки → Тачпад). Курсор в этих полосах не двигается: используется
  та же штатная curtain-зона Windows, что и у мёртвой зоны снизу. Движение, начатое в центре
  панели, продолжается в полосы как обычно и ползунком не становится — одним движением делается
  что-то одно. Двумя пальцами по-прежнему прокрутка. Чувствительность — три пресета, яркость и
  громкость идут в ногу. Нужен Precision Touchpad. На тачпадах с вендорским каналом Xiaomi (тем
  же, через который меняется сила вибрации, см. ниже) каждый шаг сопровождается коротким
  щелчком мотора, как в Xiaomi PC Manager, — включено по умолчанию. Рядом с ползунками: тумблер,
  сила щелчка (слабый / средний / сильный; отдельно от вибрации при нажатии и тоже хранится в
  тачпаде) и частота (каждый шаг / через шаг / редко, как в PC Manager; любое значение 50–500 мс
  — через `TouchpadEdgeHapticsMs` в `config.json`).
...
- **Лишние частоты можно исключить из перебора.** Панель отдаёт все свои режимы подряд, и на
  экране с 48/60/75/100/120 Гц до нужного значения приходится жать клавишу четырежды, каждый раз
  проходя настоящую смену видеорежима с чёрным кадром. В Настройки → Экран → «Перебор по клавише»
  список частот **этой** панели с тумблерами: оставь те, между которыми переключаешься. Если
  оставить меньше двух, перебор вернётся ко всем — клавиша, которая молча перестала работать,
  выглядит поломкой, а не настройкой. На панелях с двумя режимами (например 60/120 у Book Pro 14)
  раздела нет: выбирать там нечего.
...
### Сила вибрации и усилие нажатия тачпада

**Настройки → Тачпад → «Тактильный отклик»**: насколько ощутимо тачпад щёлкает под пальцем
(слабая / средняя / сильная) и как сильно нужно надавить (лёгкое / среднее / тугое; «среднее» в
обоих списках — заводская настройка). Это то же, что делает Xiaomi PC Manager, тем же вендорским
каналом тачпада — обычные HID-отчёты, без драйверов. Протокол восстановлен из кода самого
PC Manager и проверен на TM2424.

Значения хранятся в самом тачпаде, поэтому переживают сон и перезагрузку и в `config.json` не
попадают: приложение читает их один раз на старте и пишет только по вашему выбору, проверяя
каждую запись чтением обратно. На ноутбуке с другим тачпадом раздела просто нет.

**Сильное нажатие.** Продавите тачпад заметно сильнее обычного клика — выполнится любое
действие из того же списка, что у клавиш, плюс «Скриншот (Win+Shift+S)» (Настройки → Тачпад →
«Сильное нажатие», `TouchpadHeavyPressAction` в `config.json`). Как в Xiaomi PC Manager, где на
нём скриншот. Нажатие распознаётся по давлению пальца, которое тачпад и так сообщает (обычный
клик ≈ 130, сильное нажатие ≈ 1000, порог 500), а пока действие выбрано, прошивка щёлкает второй
раз — чувствуется, что «дожали». Обычный клик при этом тоже срабатывает. С «Нет действия»
ничего лишнего не работает: давление даже не разбирается.
...
устройство» в Диспетчере, без драйверов) и по умолчанию **включаются сами после
перезагрузки** — залипнуть выключенными не могут. Их ячейки есть и в быстрой панели, рядом
с авто-герцовкой (ячейка сенсорного экрана — только если тачскрин в системе присутствует).

Если устройство не нужно вовсе — в Настройки → Функции есть тумблер **«Помнить выключенным
между перезагрузками»** (свой у тачпада и у сенсорного экрана). Тогда включать обратно на
старте мы не будем; вернуть устройство можно тем же тумблером в панели или меню. Само же
«выключено не нами» приложение не трогает никогда: погасили экран Диспетчером устройств —
так он и останется.
...
`AiKey`, `ProjKey` и `TouchpadHeavyPress` для сильного нажатия), значения действий: `modes`,
`charge`, `panel`, `owl`, `monitor`, `travel`, `touchpad`, `touchscreen`, `autobright`, `hz`, `projection`,
`screenshot`, `settings`, `copilot`, `play`, `next`, `prev`, `stop`, `calc`, `launch`, `none`:
...
Готовые команды с твоим портом лежат прямо на вкладке — внизу, в разделе «Примеры запросов»:
скопировал, подставил токен, работает.

**Вебхук: событие наружу.** Обратная сторона API — XiControl сам сообщает чужой системе, что
**заряд дошёл до порога**: умная розетка выключает питание без участия человека, а Home Assistant
перестаёт опрашивать нас по кругу. Адрес получателя и тумблер — там же, на вкладке; кнопка
«Проверить» шлёт тестовое событие тем же путём и с тем же телом, что и настоящее. Тело — те же
поля, что у `GET /status`, плюс повод и порог:
...
`hardwareLimit` говорит, чей это порог, и для сценария разница существенная: `true` — заряд уже
остановила прошивка (выключить розетку полезно, но не срочно), `false` — порог **программный**,
и заряд **продолжается**, пока розетка не выключится. Имя события в обоих случаях одно: сценарий,
написанный на `chargeLimit`, не должен молча перестать видеть второй случай — а он опаснее.

Срабатывает **один раз за зарядку**; событие взводится заново, когда зарядник отключили.
Работает независимо от самого сервера: открывать слушающий сокет ради исходящего события не нужно.
Адрес не задан — ни одного сетевого запроса не делается (то же обещание, что у проверки обновлений).
Ходим только по `http`/`https`, за редиректами не идём, ответ не читаем — хватает кода состояния.
...
- Порог «беречь батарею» выбирается из дискретного набора прошивки — произвольный процент
  ...
  **А бывает, что аппаратного лимита нет вовсе** — на TM2113 прошивка команду не принимает.
  Там приложение предлагает программный порог: предупредит, что пора выдернуть провод, но
  остановить зарядку не сможет (см. «Защита заряда» выше).
